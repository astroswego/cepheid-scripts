...
#    tail -n +2 $origfile > $outfile
	...
	then
	    ...
	    highestcol=$replaceindex
	fi
	# count columns in data file to make sure there are enough
	inputcols=$(head -n 1 $inputfile | wc -w)
	if [ $inputcols -lt $replacecount ]
	then
	    echo \
		"Number of columns in $inputfile is less than number requested"
	    exit 1
	fi
...
#	((colshift += replacecount))
    ...
    echo "\
Usage: colreplace [existing table]
                  ([input table] [column to insert] [columns to insert])*
